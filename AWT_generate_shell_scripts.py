...
# d = d[d.lineage.str.contains('coggeri') | d.lineage.str.contains('amicula$')]
d = d[d.lineage.str.contains('coggeri_S')]
...
# for ix, sample in enumerate(samps):
	...
	# o.write("python ~/AWT_delimit/align_reads2.py --lineage %s --file %s --dir %s --samtools ~/bin/samtools-1.3.1/samtools --gatk ~/bin/GenomeAnalysisTK.jar --dp 20 --qual 40 --CPU %s --mem %s" % (lineage, file, outdir, cpu, mem))
	...
	# o.write("python ~/AWT_delimit/call_variants.py --lineage %s --file %s --dir %s --gatk ~/bin/GenomeAnalysisTK.jar --mem %s --CPU %s" % (lineage, file, outdir, mem, cpu))
