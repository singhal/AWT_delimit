...
outfile = os.path.join(indir, 'pop_gen', 'dstat_%s_alns.csv' % args.name)
...
def get_variants(indir, sps):
	...
	alndir = os.path.join(indir, 'original_approach/%s/loci' % args.name)
	loci = glob.glob(alndir + '/*aln')

	for locus in loci:
		seq, seqlen = get_seq(locus)
		ids = sorted(list(seq.keys()))

		c = re.sub('^.*/', '', locus)
		c = re.sub('.fasta.aln', '', c)
		...
		for pos in range(0, seqlen):
			...
			for id in ids:
				id1 = re.sub('_\d$', '', id)
				if id1 not in var[c][pos]:
					var[c][pos][id1] = []

				bp = seq[id][pos]
				if bp == '-':
					bp = 'N'
				var[c][pos][id1].append(bp)
